...
def greet ():
    if "person" in request.args:
        usr = request.args ["person"]
        #nem = request.args ["name"]
        return render_template ("greet.html", person=usr, name=5*5)
    else:
        return render_template ("greet.html", person = "Send your user name with 'person' param in query string")
